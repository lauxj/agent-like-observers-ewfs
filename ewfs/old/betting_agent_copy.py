...
#Build all circuits for betting agent:
def build_measurement(A_setting, B_setting, alpha, beta1, beta2):
    ...
    qc = QuantumCircuit(qr_SC, qr_W0, qr_W1, qr_M1, qr_M2, qr_SD, cr)
    ...
    qc.barrier()
    qc.cx(qr_M1[0], qr_W0[0])      # Bet (store in W0)
    qc.barrier()
    ...
    qc.cx(qr_M2[0], qr_W1[0])      # Bookkeeping (store in W1)
    qc.barrier()
